...
/**
 * REMOTE DATA SOURCE - Direct API access to Supabase for expenses.
 * For offline-first reads, the ExpenseRepository wrapper uses ExpenseDao.
 * This class is kept for write operations and sync with the server.
 */
class ExpenseRemoteDataSource private constructor(private val context: Context) {
    ...
    companion object {
        @Volatile
        private var instance: ExpenseRemoteDataSource? = null

        fun getInstance(context: Context): ExpenseRemoteDataSource {
            return instance ?: synchronized(this) {
                instance ?: ExpenseRemoteDataSource(context.applicationContext).also { instance = it }
            }
        }
    }
