...
async function handlePaymentIntentSucceeded(
  ...
  if (supabase_user_id && price_type?.includes("individual")) {
    ...
    if (currentType === "LICENSED") {
      console.error(`❌ BLOCKED: User ${supabase_user_id} is LICENSED and cannot purchase Individual`)
      console.error(`   Payment received but subscription NOT activated. Manual refund required.`)
      await recordBlockedPayment(supabase, supabase_user_id, paymentIntent, "BLOCKED: User is LICENSED")
      return
    }

    // Rule 2: LIFETIME users cannot purchase anything (they already have the best plan)
    if (currentType === "LIFETIME") {
      console.error(`❌ BLOCKED: User ${supabase_user_id} already has LIFETIME and cannot purchase again`)
      console.error(`   Payment received but subscription NOT activated. Manual refund required.`)
      await recordBlockedPayment(supabase, supabase_user_id, paymentIntent, "BLOCKED: User already has LIFETIME")
      return
    }
    ...
    if (currentType === "PREMIUM" && price_type === "individual_monthly") {
      console.error(`❌ BLOCKED: User ${supabase_user_id} already has active PREMIUM subscription`)
      console.error(`   Duplicate subscription attempt. Payment received but NOT activated.`)
      await recordBlockedPayment(supabase, supabase_user_id, paymentIntent, "BLOCKED: User already has active PREMIUM")
      return
    }
    ...
    if (currentType === "TRIAL" && price_type === "individual_trial") {
      ...
      await recordBlockedPayment(supabase, supabase_user_id, paymentIntent, "BLOCKED: Invalid TRIAL purchase attempt")
      return
    }
    ...
    console.log(`✅ User ${supabase_user_id} validated for Individual ${price_type} (current: ${currentType})`)
  }
  ...
  if (supabase_pro_account_id && (price_type === "pro_license_monthly" || price_type === "pro_license_lifetime")) {
    ...
    if (existingLicenses && existingLicenses.length > 0) {
      console.log(`Licenses already exist for subscription ref ${stripeSubscriptionRef}, skipping creation (found ${existingLicenses.length})`)
    } else {
      ...
      if (proError || !proAccount) {
        console.error(`❌ BLOCKED: Pro account ${supabase_pro_account_id} not found`)
        await recordBlockedPayment(supabase, null, paymentIntent, "BLOCKED: Pro account not found")
        return
      }
      ...
      if (supabase_user_id && proAccount.user_id !== supabase_user_id) {
        console.error(`❌ BLOCKED: User ${supabase_user_id} is not the owner of pro_account ${supabase_pro_account_id}`)
        console.error(`   Expected owner: ${proAccount.user_id}`)
        await recordBlockedPayment(supabase, supabase_user_id, paymentIntent, "BLOCKED: User is not pro_account owner")
        return
      }
      ...
      if (proAccount.status === "suspended") {
        console.error(`❌ BLOCKED: Pro account ${supabase_pro_account_id} is suspended due to payment failure`)
        console.error(`   The pro owner must resolve the failed payment before purchasing additional licenses`)
        await recordBlockedPayment(supabase, supabase_user_id, paymentIntent, "BLOCKED: Pro account is suspended - resolve payment failure first")
        return
      }
      ...
      if (proAccount.status === "expired") {
        console.log(`ℹ️ Pro account ${supabase_pro_account_id} was expired, reactivating with this purchase`)
      }

      // Log current status for audit trail
      console.log(`📋 Pro account ${supabase_pro_account_id} status: ${proAccount.status} (allowed for purchase)`)

      // Validate quantity limit (anti-abuse)
      const MAX_LICENSES_PER_PURCHASE = 100
      if (qty > MAX_LICENSES_PER_PURCHASE) {
        console.error(`❌ BLOCKED: Quantity ${qty} exceeds max ${MAX_LICENSES_PER_PURCHASE}`)
        await recordBlockedPayment(supabase, supabase_user_id, paymentIntent, `BLOCKED: Quantity ${qty} exceeds limit`)
        return
      }
      ...
      console.log(`📊 Pro account ${supabase_pro_account_id} currently has ${currentLicenseCount || 0} licenses, adding ${qty} more`)
      ...
      console.log(`✅ Created ${qty} ${isLifetime ? 'lifetime' : 'monthly'} licenses for pro account ${supabase_pro_account_id} (status: available)`)
      ...
      if (proAccount.status === "trial" || proAccount.status === "expired") {
        ...
        console.log(`✅ Pro account ${supabase_pro_account_id} activating (was: ${proAccount.status})`)
      }
      ...
      if (!proAccount.billing_anchor_day) {
        ...
        console.log(`📅 Setting billing_anchor_day to ${anchorDay} for pro account ${supabase_pro_account_id}`)
      }
...
async function handleCheckoutSessionCompleted(
  ...
  if (supabase_user_id && price_type?.includes("individual")) {
    ...
    if (currentType === "LICENSED") {
      console.error(`❌ BLOCKED: User ${supabase_user_id} is LICENSED and cannot purchase Individual subscription`)
      console.error(`   They must first unlink from their Pro account`)
      return
    }

    if (currentType === "LIFETIME") {
      console.error(`❌ BLOCKED: User ${supabase_user_id} already has LIFETIME - no purchase needed`)
      return
    }
    ...
    console.log(`✅ User ${supabase_user_id} validated for Individual subscription (current: ${currentType})`)
  }
...
async function handleInvoicePaid(
  ...
  if (supabase_pro_account_id && price_type === "pro_license_monthly") {
    console.log(`🔄 === PRO LICENSE RENEWAL START for account ${supabase_pro_account_id} ===`)
    ...
    console.log(`🔄 === PRO LICENSE RENEWAL COMPLETE for account ${supabase_pro_account_id} ===`)
  }
...
async function handleInvoicePaymentFailed(
  ...
  if (supabase_user_id && price_type?.includes("individual")) {
    console.log(`⚠️ Individual payment failed - setting user ${supabase_user_id} to EXPIRED`)
    console.log(`   stripe_subscriptions.status = past_due, users.subscription_type = EXPIRED`)
...
async function handleSubscriptionUpdate(
  ...
  if (supabase_user_id && price_type?.includes("individual") && subscription.status === "active") {
    const { data: user } = await supabase
      .from("users")
      .select("subscription_type")
      .eq("id", supabase_user_id)
    ...
    if (!existingUserSub) {
      ...
      if (currentType === "LICENSED") {
        console.error(`❌ BLOCKED: User ${supabase_user_id} is LICENSED and cannot create Individual subscription`)
        console.error(`   Subscription ${subscription.id} will NOT be synced. Manual intervention required.`)
        return
      }

      // Rule 2: LIFETIME cannot create any subscription
      if (currentType === "LIFETIME") {
        console.error(`❌ BLOCKED: User ${supabase_user_id} already has LIFETIME`)
      ...
      if (currentType === "PREMIUM" && price_type === "individual_monthly") {
        console.error(`❌ BLOCKED: User ${supabase_user_id} already has active PREMIUM`)
        console.error(`   Duplicate subscription ${subscription.id} will NOT be synced.`)
        return
      }

      console.log(`✅ User ${supabase_user_id} validated for new ${price_type} subscription (current: ${currentType})`)
    }
  }
  ...
  if (supabase_user_id && price_type?.includes("individual")) {
    ...
    if (subscription.cancel_at_period_end && periodEnd) {
      console.log(`📅 User ${supabase_user_id} scheduled cancellation at period end: ${new Date(periodEnd * 1000).toISOString()}`)
  ...
  if (supabase_pro_account_id && price_type === "pro_license_monthly") {
    ...
    if (proAccountCheck?.status === "suspended") {
      console.error(`❌ BLOCKED: Pro account ${supabase_pro_account_id} is suspended - cannot create licenses`)
      console.error(`   Subscription ${subscription.id} created but licenses NOT created until payment issue resolved`)
      return
    }
    ...
    if (subscription.status === "active") {
      ...
      console.log(`📊 License count: existing=${currentLicenseCount}, target=${targetLicenseCount}, toCreate=${licensesToCreate}`)
      ...
      if (licensesWithoutSubId && licensesWithoutSubId.length > 0) {
        ...
        console.log(`🔗 Linked ${licensesWithoutSubId.length} existing licenses to subscription ${subscription.id}`)
      }
      ...
      if (licensesToCreate > 0) {
        ...
        await supabase.from("licenses").insert(licenses)
        console.log(`✅ Created ${licensesToCreate} monthly licenses for pro account ${supabase_pro_account_id} (status: available, stripe_subscription_ref: ${stripeSubscriptionRef || 'null'}), end_date: ${periodEndStr}`)
        console.log(`   Total licenses now: ${currentLicenseCount + licensesToCreate}`)
        ...
        if (proAccount) {
          ...
          if (!proAccount.stripe_subscription_id || proAccount.stripe_subscription_id !== subscription.id) {
            proUpdateData.stripe_subscription_id = subscription.id
            console.log(`🔗 Linking stripe_subscription_id ${subscription.id} to pro account ${supabase_pro_account_id}`)
          }
        ...
        console.log(`ℹ️ License count already matches subscription quantity (${targetLicenseCount}), no new licenses needed`)
      } else {
        ...
        console.warn(`⚠️ License count (${currentLicenseCount}) exceeds subscription quantity (${targetLicenseCount})`)
        console.warn(`   This may indicate manual licenses or a downgrade - no automatic deletion`)
      }
    }
  }
}
...
async function processSubscriptionDeleted(
  ...
  try {
    ...
    if (error) {
      console.error(`❌ Error calling process_subscription_deleted RPC:`, error)
      // Fallback to legacy processing
      console.log(`⚠️ Falling back to legacy processing...`)
      await processSubscriptionDeletedLegacy(supabase, stripeSubscriptionId, proAccountId)
      return
    }
    ...
    if (data?.success) {
      ...
      console.error(`❌ process_subscription_deleted returned failure:`, data?.error)
      await processSubscriptionDeletedLegacy(supabase, stripeSubscriptionId, proAccountId)
    }
  } catch (err) {
...
async function processSubscriptionDeletedLegacy(
  ...
  console.log(`🔴 [LEGACY] Processing subscription deletion for ${stripeSubscriptionId}`)
  console.warn(`⚠️ [LEGACY] Using non-atomic fallback - RPC should be deployed for production`)
  ...
  if (!licenses || licenses.length === 0) {
    console.log(`ℹ️ [LEGACY] No licenses to process (may be already processed)`)
    return
  }
  ...
  if (affectedUserIds.length > 0) {
    ...
    console.log(`📋 [LEGACY] Checking ${uniqueUserIds.length} affected users...`)
    ...
    for (const userId of uniqueUserIds) {
      ...
      if (checkError) {
        console.error(`❌ [LEGACY] Failed to check other licenses for user ${userId}:`, checkError)
        continue  // Skip this user, don't set to EXPIRED without confirmation
      }

      if (otherActiveLicenses && otherActiveLicenses.length > 0) {
        console.log(`ℹ️ [LEGACY] User ${userId} has another active license - keeping LICENSED`)
      } else {
        ...
        if (userUpdateError) {
          console.error(`❌ [LEGACY] Failed to update user ${userId}:`, userUpdateError)
        } else {
          console.log(`⚠️ [LEGACY] User ${userId} set to EXPIRED (no other active licenses)`)
        }
      }
    }
  }
...
async function recordBlockedPayment(
  ...
  console.error(`🚨 Amount: ${paymentIntent.amount / 100} ${paymentIntent.currency.toUpperCase()}`)
  ...
  console.log(`📋 To process refund, run: stripe refunds create --payment-intent ${paymentIntent.id}`)
}
...
async function processProRenewal(supabase: any, proAccountId: string) {
  console.log(`🔄 Processing Pro renewal for account ${proAccountId} via RPC...`)
  ...
  try {
    ...
    if (data?.success) {
      ...
      console.log(`   - Returned to pool: ${data.returned_count || 0} lifetime licenses`)
      console.log(`   - Affected users: ${data.affected_users || 0}`)
    } else {
...
async function processProRenewalLegacy(supabase: any, proAccountId: string) {
  ...
  if (canceledError) {
    console.error(`❌ [LEGACY] Failed to fetch canceled licenses:`, canceledError)
  ...
  if (unlinkError) {
    console.error(`❌ [LEGACY] Failed to fetch unlink-expired licenses:`, unlinkError)
  }

  // Merge and deduplicate
  const allLicenses = [...(canceledLicenses || []), ...(unlinkExpiredLicenses || [])]
  ...
  if (!licensesToProcess || licensesToProcess.length === 0) {
    console.log(`ℹ️ [LEGACY] No canceled/unlinked licenses to process (may be already processed)`)
    return
  }

  console.log(`📋 [LEGACY] Found ${licensesToProcess.length} licenses to process`)
  ...
  for (const license of licensesToProcess) {
    ...
    if (license.linked_account_id) {
      ...
      if (linkError) {
        console.error(`❌ [LEGACY] Failed to deactivate company_link for user ${license.linked_account_id}:`, linkError)
    ...
    if (license.is_lifetime) {
      ...
      if (updateError) {
        console.error(`❌ [LEGACY] Failed to return lifetime license ${license.id} to pool:`, updateError)
      } else {
        console.log(`✅ [LEGACY] Lifetime license ${license.id} returned to pool`)
      ...
      if (deleteError) {
        console.error(`❌ [LEGACY] Failed to delete monthly license ${license.id}:`, deleteError)
      } else {
  ...
  if (affectedUserIds.length > 0) {
    const uniqueUserIds = [...new Set(affectedUserIds)]
    console.log(`📋 [LEGACY] Checking license status for ${uniqueUserIds.length} affected users...`)
    ...
    for (const userId of uniqueUserIds) {
      ...
      if (otherActiveLicenses && otherActiveLicenses.length > 0) {
        console.log(`ℹ️ [LEGACY] User ${userId} has another active license (${otherActiveLicenses[0].id}) - keeping LICENSED status`)
      } else {
        // AUDIT FIX: WHERE subscription_type = 'LICENSED' prevents double-updates
        ...
        if (userUpdateError) {
          ...
          console.log(`⚠️ [LEGACY] User ${userId} has no more active licenses - set to EXPIRED`)
        }
      }
    }
  }

  console.log(`✅ [LEGACY] Pro renewal complete: processed ${processedLicenseIds.length} licenses`)
}
  ...
  try {
    ...
    if (error) {
      ...
      await processInvoicePaidProLegacy(
        supabase, proAccountId, stripeSubscriptionId, periodEnd,
    ...
    if (data?.success) {
      ...
      console.error(`❌ process_invoice_paid_pro returned failure:`, data?.error)
      await processInvoicePaidProLegacy(
        supabase, proAccountId, stripeSubscriptionId, periodEnd,
        stripeSubscriptionItemId, stripePriceId
      )
...
async function processInvoicePaidProLegacy(
  ...
  if (suspendedLicenses && suspendedLicenses.length > 0) {
    console.log(`   ✅ [LEGACY] Reactivated ${suspendedLicenses.length} suspended licenses`)
  }
  ...
  if (error) {
    ...
    console.log(`   ✅ [LEGACY] Renewed ${updatedLicenses?.length || 0} licenses`)
    ...
    if (assignedLicenses && assignedLicenses.length > 0) {
      const userIds = assignedLicenses.map((l: any) => l.linked_account_id).filter(Boolean)
      for (const userId of userIds) {
        await supabase.from("users")
          .update({ subscription_expires_at: periodEnd, updated_at: new Date().toISOString() })
          .eq("id", userId)
      }
      console.log(`   ✅ [LEGACY] Updated expiration for ${userIds.length} users`)
    }
  }
